...
class Post {
  ...
  //Factory constructon untuk membuat objek Post dari data JSON
  factory Post.fromJson(Map<String, dynamic> json) {
    return Post(
      id: json['id'],
      userId: json['userId'] ?? 1, //Memberikan nilai default jika null
      title: json['title'],
      body: json['body'],
...
class _HomePageState extends State<HomePage> {
  ...
  //Letakkan kode ini di dalam class _HomePageState, di atas method build
  Future<void> createPost(String title, String body) async {
    ...
    try {
      ...
      //Pengecekkan untuk memastikan widget masih ada di tree sebelum update state
      if (!mounted) return;
      ...
      if (response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Gagal membuat post')));
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> updatePost(int id, String title, String body) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          //Update state 'createPost' dengan data baru dari server.
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Gagal memperbarui post')));
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> deletePost(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        //Aturan state menjadi null untuk mengembaikan UI ke keadaan semua
        setState(() {
          createdPost = null;
        });
      } else {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              final updateTitle = titleController.text.trim();
              final updatedBody = bodyController.text.trim();

              if (updateTitle.isNotEmpty && updatedBody.isNotEmpty) {
                updatePost(currentPost.id, updateTitle, updatedBody);
                Navigator.pop(context);
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
    ...
    return Scaffold(
      ...
      body: Center(
        child: createdPost == null
            ...
            : ListView(
                //Tampilan jika data suda dibuat
